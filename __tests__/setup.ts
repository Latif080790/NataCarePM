...
const mockServices = {
    intelligentDocumentService: {
        ...
        createDocument: jest.fn().mockImplementation(async (title, description, category, projectId, userId, file, templateId, options) => {
            const document = {
        ...
        autoGenerateDocument: jest.fn().mockImplementation(async (templateId, data, projectId, userId) => {
        ...
        initiateSignatureWorkflow: jest.fn().mockImplementation(async (documentId, signers, sequential, deadline, userId) => {
            return {
