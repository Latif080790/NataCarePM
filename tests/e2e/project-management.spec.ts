...
test.describe('Project Management Flow', () => {
  ...
  test('should create a new project successfully', async ({ page }) => {
    ...
    await expect(page.getByText(/project created|successfully created/i)).toBeVisible({ timeout: 5000 });
  ...
  test('should handle project permissions correctly', async ({ page }) => {
    ...
    const hasEditButton = await page.getByRole('button', { name: /edit/i }).isVisible({ timeout: 3000 });
    const hasDeleteButton = await page.getByRole('button', { name: /delete/i }).isVisible({ timeout: 3000 });
