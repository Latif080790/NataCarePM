...
if (!validation.valid) {
  return wrapError(
    new APIError(ErrorCodes.VALIDATION_FAILED, 'Task validation failed', 400, {
...
if (validation.warnings && validation.warnings.length > 0) {
  logger.warn({ component: 'taskService', action: 'createTask' }, 'Task has warnings', {
...
const ocrCircuitBreaker = new CircuitBreaker(() => ocrService.processDocument(file), {
  failureThreshold: 5,
