...
export class AccountsPayableService {
     ...
     */
    private calculateAging(invoiceDate: Date): { agingDays: number; agingBracket: '0-30' | '31-60' | '61-90' | '90+' } {
        const now = new Date();
    ...
    async createAccountsPayable(
        apData: Omit<AccountsPayable, 'id' | 'apNumber' | 'createdAt' | 'createdBy' | 'updatedAt' | 'updatedBy' | 'amountPaid' | 'amountDue' | 'status' | 'agingDays' | 'agingBracket' | 'payments'>,
        userId: string
    ...
    async getAccountsPayableByStatus(status: PayableStatus): Promise<AccountsPayable[]> {
        try {
            ...
            logger.error('getAccountsPayableByStatus', 'Failed to get AP by status', error as Error, { status });
            return [];
        }
    }
    ...
    async getAccountsPayableByVendor(vendorId: string): Promise<AccountsPayable[]> {
        try {
            ...
            logger.error('getAccountsPayableByVendor', 'Failed to get AP by vendor', error as Error, { vendorId });
            return [];
        }
    }
     ...
     */
    async approveAccountsPayable(apId: string, userId: string, notes?: string): Promise<AccountsPayable | null> {
        try {
    ...
    async recordPayment(
        apId: string,
        payment: Omit<Payment, 'id' | 'paymentNumber' | 'createdAt' | 'createdBy' | 'referenceType' | 'referenceId'>,
        userId: string
    ...
    async generateAgingReport(): Promise<AgingReport> {
        try {
            ...
            const unpaidAP = allAP.filter(ap => 
                ap.status !== 'paid' && 
                ap.status !== 'cancelled' && 
                ap.status !== 'void'
            );
            ...
            brackets.forEach(bracket => {
                bracket.percentage = totalAmount > 0 
                    ? (bracket.totalAmount / totalAmount) * 100 
                    : 0;
            });
