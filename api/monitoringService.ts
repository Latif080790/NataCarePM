...
export class MonitoringValidator {
    ...
    static validateUserActivity(activity: Partial<UserActivity>): ValidationResult {
        ...
        // Validation patterns
        const validActions = ['create', 'read', 'update', 'delete', 'navigate', 'login', 'logout', 'export', 'import'];
    ...
    static validateErrorLog(error: Partial<ErrorLog>): ValidationResult {
        ...
        if (error.environment && !['development', 'staging', 'production'].includes(error.environment)) {
...
export class MonitoringService {
     ...
     */
    async logError(error: Omit<ErrorLog, 'errorId' | 'timestamp' | 'resolved' | 'environment'>): Promise<void> {
        try {
     ...
     */
    async logUserActivity(activity: Omit<UserActivity, 'timestamp' | 'deviceInfo' | 'sessionId'>): Promise<void> {
        try {
     ...
     */
    private async executeWithRetry(operation: () => Promise<void>, operationName: string): Promise<void> {
        if (!this.isOnline) {
        ...
        for (let attempt = 1; attempt <= this.config.retryAttempts; attempt++) {
            try {
                ...
                console.warn(`⚠️ ${operationName} attempt ${attempt}/${this.config.retryAttempts} failed:`, error);
        ...
        // All attempts failed
        console.error(`❌ ${operationName} failed after ${this.config.retryAttempts} attempts:`, lastError);
     ...
     */
    private calculateProjectRiskLevel(healthScore: number, performanceScore: number): ProjectMetrics['riskLevel'] {
        ...
        try {
            ...
            return {
                ...
                summary: {
                    ...
                    criticalErrors: errors.status === 'fulfilled' ? 
                        errors.value.filter((error: any) => error.severity === 'critical').length : 0,
                },
                timeRange,
