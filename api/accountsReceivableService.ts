...
export class AccountsReceivableService {
    ...
    async createAccountsReceivable(
        arData: Omit<AccountsReceivable, 'id' | 'arNumber' | 'createdAt' | 'createdBy' | 'updatedAt' | 'updatedBy' | 'amountPaid' | 'amountDue' | 'status' | 'agingDays' | 'agingBracket' | 'payments' | 'collectionNotes'>,
        userId: string
    ...
    async getAccountsReceivableByStatus(status: ReceivableStatus): Promise<AccountsReceivable[]> {
        try {
            ...
            logger.error('getAccountsReceivableByStatus', 'Failed to get AR by status', error as Error, { status });
            return [];
        }
    }
    ...
    async getAccountsReceivableByCustomer(customerId: string): Promise<AccountsReceivable[]> {
        try {
            ...
            logger.error('getAccountsReceivableByCustomer', 'Failed to get AR by customer', error as Error, { customerId });
            return [];
        }
    }
    ...
    async generateAgingReport(): Promise<AgingReport> {
        try {
            ...
            const unpaidAR = allAR.filter(ar => 
                ar.status !== 'paid' && 
                ar.status !== 'cancelled' && 
                ar.status !== 'void'
            );
