...
export class DocumentVersionControl {
    ...
    async mergeBranches(
        ...
        if (conflicts.length > 0) {
            const validStrategy = mergeStrategy === 'manual' || mergeStrategy === 'fast-forward' || mergeStrategy === 'recursive' ? 'ours' : mergeStrategy;
    ...
    private async detectConflicts(
        ...
        for (const sourceChange of sourceChanges) {
            const conflictingChange = targetChanges.find(targetChange => 
    ...
    private async createMergeCommit(
        ...
        const versionNumber = await this.calculateNextVersion(
            documentId,
            toBranch,
            targetVersion.id
        );
    ...
    private async getLatestVersionId(documentId: string, branchName: string): Promise<string | undefined> {
    ...
    private calculateDifferences(version1: DocumentVersion, version2: DocumentVersion): VersionDifference[] {
