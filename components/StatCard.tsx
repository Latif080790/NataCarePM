  ...
  return (
    ...
    >
      ...
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3 relative z-10">
        <CardTitle className="text-sm font-semibold text-palladium group-hover:text-night-black transition-colors duration-300">{title}</CardTitle>
        <div className="w-12 h-12 rounded-xl glass flex items-center justify-center group-hover:scale-110 transition-all duration-300 border border-violet-essence/20">
          <Icon className={`h-5 w-5 ${color} group-hover:text-precious-persimmon transition-colors duration-300`} />
        </div>
      </CardHeader>
      
      <CardContent className="relative z-10">
        <div className="flex items-end justify-between mb-3">
          <div className="text-3xl font-bold gradient-text group-hover:scale-105 transition-transform duration-300">{value}</div>
          {trend !== undefined && (
            <div className={`flex items-center space-x-1 text-sm ${getTrendColor(trend)} px-2 py-1 rounded-lg glass-subtle`}>
              {getTrendIcon(trend)}
