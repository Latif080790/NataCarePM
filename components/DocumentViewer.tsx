...
export const DocumentViewer: React.FC<DocumentViewerProps> = ({ 
    ...
    const getSignatureStatus = (signature: DigitalSignature) => {
        const now = new Date();
        if (signature.isRevoked) return { status: 'revoked', color: 'text-red-600', icon: <FileX className="w-4 h-4" /> };
        if (signature.expiresAt && now > signature.expiresAt) return { status: 'expired', color: 'text-gray-600', icon: <Clock className="w-4 h-4" /> };
        if (signature.isValid) return { status: 'valid', color: 'text-green-600', icon: <CheckCircle className="w-4 h-4" /> };
        return { status: 'invalid', color: 'text-red-600', icon: <AlertTriangle className="w-4 h-4" /> };
    };
        ...
        <div className="bg-white border-b border-gray-200">
            <div className="flex space-x-8 px-6">
                {[
                    ...
                    { id: 'insights', label: `AI Insights (${document.aiInsights.length})`, icon: <Brain className="w-4 h-4" /> },
                    { id: 'versions', label: `Versions (${document.allVersions.length})`, icon: <History className="w-4 h-4" /> },
                    { id: 'signatures', label: `Signatures (${document.signatures.length})`, icon: <Signature className="w-4 h-4" /> },
        ...
        <div className="p-6">
            <Card>
                <CardContent className="p-6">
                    ...
                    {/* Structured Data */}
                    {document.ocrResults?.[0]?.structuredData && Object.keys(document.ocrResults[0].structuredData).length > 0 && (
                        <div className="mb-6">
                    ...
                    <div>
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                            <div className="bg-gray-50 rounded-lg p-3">
                                ...
                                <div className="text-sm text-gray-900">{document.fileSize ? `${(document.fileSize / 1024 / 1024).toFixed(2)} MB` : 'N/A'}</div>
                            </div>
                            <div className="bg-gray-50 rounded-lg p-3">
            ...
            <div className="space-y-4">
                                ...
                                {isExpanded && (
                                    <div className="mt-4 pt-4 border-t border-gray-100">
                                        ...
                                        <div className="text-xs text-gray-500">
                                            Generated on {insight.generatedAt ? new Date(insight.generatedAt).toLocaleString() : 'Unknown'}
                                        </div>
                                    </div>
                                )}
                            </CardContent>
                ...
                {document.aiInsights.length === 0 && (
                    <div className="text-center py-8">
                        ...
                        <p className="text-gray-500">AI analysis will appear here once the document is processed.</p>
                    </div>
                )}
            </div>
        </div>
                                ...
                                <div className="flex items-center space-x-3">
                                    ...
                                    <div>
                                        <h4 className="font-medium text-gray-900">
                                            ...
                                            {index === 0 && <span className="ml-2 text-xs text-blue-600 font-normal">(Current)</span>}
                                        </h4>
                            ...
                            <CardContent className="p-4">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <div className="text-xs text-gray-500">
                                            {signature.signedAt ? new Date(signature.signedAt).toLocaleString() : 'Pending'}
                                        </div>
                                    </div>
                                </div>
