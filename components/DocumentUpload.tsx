...
export const DocumentUpload: React.FC<DocumentUploadProps> = ({ 
    ...
    const getStatusIcon = (status: UploadFile['status']) => {
        switch (status) {
            ...
            case 'processing': return <Brain className="w-5 h-5 text-purple-500 animate-pulse" />;
                ...
                <CardContent className="p-0">
                    ...
                    >
                        ...
                        <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-2 text-xs text-gray-500">
                            {Object.entries(supportedTypes).slice(0, 8).map(([type, name]) => (
                    ...
                    <CardHeader className="flex flex-row items-center justify-between">
                        ...
                        <div className="flex items-center space-x-2">
                            <Button
                                ...
                                disabled={isProcessing || uploadFiles.filter(f => f.status === 'pending').length === 0}
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center justify-between mt-1">
                                                <p className="text-xs text-gray-500">
                                                    {formatFileSize(uploadFile.file.size)} • {supportedTypes[uploadFile.file.type as keyof typeof supportedTypes] || 'Unknown type'}
                                                </p>
                                                <p className="text-xs text-gray-500 capitalize">
                                                    {uploadFile.status === 'processing' ? 'AI Processing...' : uploadFile.status}
                                                </p>
                                            </div>
                                            ...
                                            {uploadFile.status === 'completed' && uploadFile.document && (
                                                <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded text-xs text-green-700">
                                                    ...
                                                    {enableOCR && (
                                                        <span className="ml-2">• OCR extracted {uploadFile.document.ocrResults?.[0]?.extractedText?.length || 0} characters</span>
                                                    )}
                                                    {enableAIProcessing && (
                                                        <span className="ml-2">• {uploadFile.document.aiInsights.length} AI insights generated</span>
                                                    )}
                                                </div>
                                            )}
                                        </div>
