...
export default function AiAssistantChat() {
    ...
    const storageKey = useMemo(() => `ai:chat:${currentProject?.id || 'no-project'}`, [currentProject?.id]);
        ...
        if (isOpen) {
            ...
            if (!apiKey) {
                setHistory(prev => [...prev, {
                    ...
                    role: 'model',
                    parts: [{ text: 'Konfigurasi API key tidak ditemukan. Set variabel lingkungan VITE_GEMINI_API_KEY di .env.local untuk mengaktifkan AI.' }],
    ...
    const handleSend = async () => {
        ...
        const userMessage: ChatMessage = { id: crypto.randomUUID(), role: 'user', parts: [{ text: input }], createdAt: now };
        ...
        try {
            ...
            setHistory(prev => [...prev, { id: modelMsgId, role: 'model', parts: [{ text: '' }], createdAt: new Date().toISOString() }]);

            for await (const chunk of stream) {
                const textChunk = chunk.text || chunk.candidates?.[0]?.content?.parts?.map((p: any) => p.text).join('') || '';
                modelResponse += textChunk;
                setHistory(prev => {
                    const newHistory = [...prev];
                    newHistory[newHistory.length - 1] = { id: modelMsgId, role: 'model', parts: [{ text: modelResponse }], createdAt: new Date().toISOString() };
                    return newHistory;
                });
            }
            ...
            setHistory(prev => [...prev, { id: crypto.randomUUID(), role: 'model', parts: [{ text: 'Maaf, terjadi kesalahan saat menghubungi AI.' }], createdAt: new Date().toISOString() }]);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const createTaskFromText = async (text: string) => {
        ...
        try {
            ...
            const taskId = await taskService.createTask(currentProject.id, {
                projectId: currentProject.id,
                title,
                description: text,
                status: 'todo',
            ...
            setHistory(prev => [...prev, {
                id: crypto.randomUUID(), role: 'model', parts: [{ text: `Gagal membuat task: ${e?.message || e}` }], createdAt: new Date().toISOString()
            }]);
        }
    };
                    ...
                    <Card className="w-full h-full flex flex-col shadow-2xl">
                        <CardHeader className="flex flex-row items-center justify-between border-b">
                            ...
                            <div className="flex items-center gap-1">
                                <Button variant="ghost" size="icon" title="Export transkrip" onClick={exportTranscript}><HistoryIcon className="w-5 h-5"/></Button>
                                <Button variant="ghost" size="icon" title="Bersihkan riwayat" onClick={clearHistory}><Trash2 className="w-5 h-5"/></Button>
                        ...
                        <div className="px-4 pt-3 border-b bg-gray-50">
                            ...
                            <div className="flex flex-wrap gap-2 pb-3">
                                {suggestedPrompts.map((p, i) => (
                                    <button key={i} className="px-2 py-1 rounded-full bg-white border text-xs hover:bg-persimmon hover:text-white transition"
                        ...
                        <div className="p-4 border-t space-y-2">
                            ...
                            <div className="flex items-center gap-2 text-xs">
                                <Button variant="outline" size="sm" disabled={!currentProject || !currentUser || isLoading}
                                    onClick={() => createTaskFromText(input || history.filter(h=>h.role==='model').at(-1)?.parts[0].text || 'Task dari AI')}
                                >
