...
export function usePermissions() {
     ...
     */
    const isOwner = useCallback((resource: any): boolean => {
     ...
     */
    const getAccessibleActions = useCallback((resourceType: ResourceType): Action[] => {
     ...
     */
    const canManageUser = useCallback((targetUser: any): boolean => {
...
export function useRoleCheck() {
    ...
    const isMinRole = useCallback((minRoleName: string): boolean => {
    ...
    const isMaxRole = useCallback((maxRoleName: string): boolean => {
        if (!currentUser) return false;
