...
export class ZeroTrustAuthService {
  ...
  async authenticateWithZeroTrust(
    ...
    const deviceTrust = await this.deviceTrustService.assessDevice(deviceFingerprint, user.id);
  ...
  private getUserCapabilities(user: any, riskScore: number): string[] {
    ...
    if (riskScore > 0.7) {
      return baseCapabilities.filter(
        (cap) => !['admin:delete', 'finance:modify', 'system:config'].includes(cap)
      );
    }

    if (riskScore > 0.5) {
      return baseCapabilities.filter(
        (cap) => !['admin:delete', 'finance:bulk-modify'].includes(cap)
      );
    }
...
export class DeviceTrustService {
  ...
  async assessDevice(fingerprint: DeviceFingerprint, userId: string): Promise<DeviceTrustResult> {
    // Check if device is known and trusted
    const knownDevice = await this.deviceRepository.findByFingerprint(fingerprint.hash, userId);
  ...
  private detectSuspiciousPlugins(plugins: string[]): boolean {
    const suspiciousPlugins = ['WebDriver', 'Selenium', 'Phantom', 'wdio', 'chromium-automation'];
...
export class BehaviorAnalysisService {
  ...
  async analyzeBehavior(userId: string, context: RequestContext): Promise<BehaviorRiskResult> {
    ...
    const anomalies = await this.detectBehaviorAnomalies(currentPatterns, historicalBehavior);
  ...
  private detectTimeAnomalies(current: TimePattern, historical: TimePattern[]): BehaviorAnomaly[] {
    const anomalies: BehaviorAnomaly[] = [];
    ...
    if (hourFrequency[current.hour] < 0.05) {
      // Less than 5% of historical logins
      anomalies.push({
  ...
  private detectLocationAnomalies(
    ...
    if (latestLocation) {
      const distance = this.calculateDistance(current.coordinates, latestLocation.coordinates);
