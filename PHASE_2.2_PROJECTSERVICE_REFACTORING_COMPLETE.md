...
await withRetry(() => addDoc(collection(db, 'path'), data), {
  maxAttempts: 3,
  onRetry: (attempt, error) => {
...
if (!validation.valid) {
  throw new APIError(ErrorCodes.VALIDATION_FAILED, validation.errors[0], 400, {
...
// Upload with retry
const uploadResult = await withRetry(() => uploadBytes(storageRef, file), { maxAttempts: 3 });
...
_This transformation demonstrates the power of comprehensive refactoring with enterprise-grade utilities. The pattern established here will enable rapid, consistent improvements across all services._
