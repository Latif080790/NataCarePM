...
interface RiskContextState {
  ...
  createRisk: (risk: Omit<Risk, 'id' | 'createdAt' | 'updatedAt' | 'riskScore' | 'priorityLevel' | 'reviewHistory' | 'statusHistory'>) => Promise<Risk>;
...
export const RiskProvider: React.FC<RiskProviderProps> = ({ children }) => {
   ...
   */
  const fetchRisks = useCallback(async (projectId: string, filterOptions?: RiskFilterOptions) => {
  ...
  const createRisk = useCallback(async (
    risk: Omit<Risk, 'id' | 'createdAt' | 'updatedAt' | 'riskScore' | 'priorityLevel' | 'reviewHistory' | 'statusHistory'>
  ): Promise<Risk> => {
    setRisksLoading(true);
    setRisksError(null);
   ...
   * Delete risk
   */
  const deleteRisk = useCallback(async (riskId: string): Promise<void> => {
    setRisksLoading(true);
    setRisksError(null);
   ...
   */
  const getRisksByPriority = useCallback((priority: RiskPriorityLevel): Risk[] => {
   ...
   */
  const getRisksByCategory = useCallback((category: RiskCategory): Risk[] => {
   ...
   * Get risks by status
   */
  const getRisksByStatus = useCallback((status: RiskStatus): Risk[] => {
   ...
   */
  const refreshRisks = useCallback(async (projectId: string): Promise<void> => {
