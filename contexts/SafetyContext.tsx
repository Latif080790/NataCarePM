...
interface SafetyContextState {
  ...
  createIncident: (incident: Omit<SafetyIncident, 'id' | 'incidentNumber' | 'createdAt' | 'updatedAt'>) => Promise<SafetyIncident>;
  ...
  createTraining: (training: Omit<SafetyTraining, 'id' | 'trainingNumber' | 'createdAt' | 'updatedAt'>) => Promise<SafetyTraining>;
  ...
  createPPEItem: (ppe: Omit<PPEInventory, 'id' | 'createdAt' | 'updatedAt'>) => Promise<PPEInventory>;
  updatePPEItem: (ppeId: string, updates: Partial<PPEInventory>) => Promise<void>;
  createPPEAssignment: (assignment: Omit<PPEAssignment, 'id' | 'createdAt' | 'updatedAt'>) => Promise<PPEAssignment>;
  
  // Actions - Audits
  fetchAudits: (projectId: string) => Promise<void>;
  createAudit: (audit: Omit<SafetyAudit, 'id' | 'auditNumber' | 'createdAt' | 'updatedAt'>) => Promise<SafetyAudit>;
  ...
  createObservation: (observation: Omit<SafetyObservation, 'id' | 'createdAt' | 'updatedAt'>) => Promise<SafetyObservation>;
...
export const SafetyProvider: React.FC<SafetyProviderProps> = ({ children }) => {
   ...
   */
  const fetchIncidentById = useCallback(async (incidentId: string): Promise<SafetyIncident | null> => {
    try {
  ...
  const updateIncident = useCallback(async (
    ...
    try {
      ...
      setIncidents(prev => prev.map(i => 
        i.id === incidentId ? { ...i, ...updates, updatedAt: new Date() } : i
      ));
      
      if (selectedIncident?.id === incidentId) {
        setSelectedIncident(prev => prev ? { ...prev, ...updates, updatedAt: new Date() } : null);
      }
    } catch (error: any) {
  ...
  const updateTraining = useCallback(async (
    ...
    try {
      ...
      setTraining(prev => prev.map(t => 
        t.id === trainingId ? { ...t, ...updates, updatedAt: new Date() } : t
      ));
      
      if (selectedTraining?.id === trainingId) {
        setSelectedTraining(prev => prev ? { ...prev, ...updates, updatedAt: new Date() } : null);
      }
    } catch (error: any) {
   ...
   * Create PPE item
   */
  const createPPEItem = useCallback(async (
   ...
   * Update PPE item
   */
  const updatePPEItem = useCallback(async (
    ppeId: string,
    ...
    try {
      ...
      setPPEInventory(prev => prev.map(p => 
        p.id === ppeId ? { ...p, ...updates, updatedAt: new Date() } : p
      ));
    } catch (error: any) {
   ...
   */
  const createPPEAssignment = useCallback(async (
  ...
  const updateAudit = useCallback(async (
    ...
    try {
      ...
      if (selectedAudit?.id === auditId) {
        setSelectedAudit(prev => prev ? { ...prev, ...updates, updatedAt: new Date() } : null);
      }
    } catch (error: any) {
  ...
  const fetchMetrics = useCallback(async (
    ...
    try {
      const calculatedMetrics = await safetyService.calculateMetrics(projectId, periodStart, periodEnd);
   ...
   */
  const getIncidentsBySeverity = useCallback((severity: IncidentSeverity): SafetyIncident[] => {
   ...
   */
  const getIncidentsByStatus = useCallback((status: IncidentStatus): SafetyIncident[] => {
   ...
   */
  const getTrainingByStatus = useCallback((status: TrainingStatus): SafetyTraining[] => {
  ...
  const getUpcomingTraining = useCallback((): SafetyTraining[] => {
    ...
    return training.filter(t => 
      t.scheduledDate > now && t.status === 'scheduled'
   ...
   */
  const getAuditsByStatus = useCallback((status: AuditStatus): SafetyAudit[] => {
   ...
   */
  const refreshSafety = useCallback(async (projectId: string): Promise<void> => {
    await Promise.all([
