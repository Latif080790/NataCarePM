...
const WBSManagementView: React.FC<WBSManagementViewProps> = ({ 
        ...
        return (
            <div key={element.id} className="mb-1">
                ...
                >
                    ...
                    {/* WBS Code */}
                    <div className="w-24 flex-shrink-0 font-mono text-sm font-semibold">
                        {element.code}
                    </div>
                    ...
                    {/* Variance */}
                    <div className={`w-32 text-right text-sm font-semibold ${getVarianceColor(element.variance)}`}>
                        {formatCurrency(Math.abs(element.variance))}
                        <div className="text-xs">
                            {element.variance >= 0 ? 'Under' : 'Over'}
                        </div>
                    </div>
                    ...
                    <div className="w-32 flex-shrink-0 px-3">
                        <span className={`px-2 py-1 text-xs rounded-full font-semibold inline-flex items-center gap-1 ${getStatusColor(element.status)}`}>
    ...
    const handleUpdate = async (formData: Partial<WBSElement>) => {
        ...
        try {
            await wbsService.updateWBSElement(
                selectedElement.id,
                formData,
                currentUser
            );
    ...
    const totals = useMemo(() => {
        ...
        return hierarchy.flatList.reduce((acc, el) => ({
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-sm text-gray-600 mt-1">
                                {projectName} • {hierarchy?.totalElements || 0} Elements • {hierarchy?.maxLevel || 0} Levels
                            </p>
                        </div>
                        {canManage && (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                        Format: 1.2.3 (number.number.number)
                    </p>
                </div>
                ...
                <div>
                    ...
                    <select
                        ...
                        onChange={(e) => setFormData({ ...formData, status: e.target.value as WBSElement['status'] })}
                        className="w-full px-3 py-2 border rounded-lg"
                    >
                        <option value="Not Started">Not Started</option>
                        <option value="In Progress">In Progress</option>
