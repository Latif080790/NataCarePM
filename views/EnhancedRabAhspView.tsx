...
export default function EnhancedRabAhspView({ items, ahspData, projectLocation }: EnhancedRabAhspViewProps) {
  ...
  const [enhancedItems, setEnhancedItems] = useState<EnhancedRabItem[]>(() => 
    items.map(item => EnhancedRabService.createEnhancedRabItem(item, {
  ...
  const handleExportCsv = () => {
    const headers = [
      ...
      "Harga Dasar", "Labor Cost", "Material Cost", "Equipment Cost",
      "Regional Adjustment", "Escalation Rate", "Risk Level", "Adjusted Price", "Total"
    ];
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-persimmon">{formatCurrency(totalEnhancedBudget)}</p>
              </div>
      ...
      <Card>
        <CardHeader className="flex flex-row justify-between items-center">
          <div>
            ...
            <CardDescription>Comprehensive cost analysis with risk assessment and regional adjustments</CardDescription>
          </div>
            ...
            <table className="w-full text-sm text-left text-night-black">
              ...
              <tbody>
                {enhancedItems.map(item => {
                  ...
                  const projectedPrice = EnhancedRabService.calculatePriceEscalation(
                    adjustedPrice, item.escalationRate, 12
                  );
                  ...
                  return (
                    <>
                      <tr key={item.id} className="border-b border-violet-essence hover:bg-violet-essence/30">
                        <td className="p-3">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              >
                                <span className="font-medium">{item.no}</span> {item.uraian || 'N/A'}
                              </button>
                              <p className="text-xs text-palladium">{item.volume} {item.satuan} • {item.kategori}</p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="p-3 text-right">
                          <span className={`${adjustedPrice !== item.hargaSatuan ? 'text-orange-600 font-medium' : 'text-palladium'}`}>
                            {((adjustedPrice / item.hargaSatuan - 1) * 100).toFixed(1)}%
                          </span>
                        </td>
                        <td className="p-3 text-right">
                          <span className={`${item.escalationRate > 10 ? 'text-red-500' : item.escalationRate > 5 ? 'text-yellow-500' : 'text-green-500'}`}>
                            {item.escalationRate.toFixed(1)}%/yr
                          </span>
                        </td>
                        <td className="p-3 text-center">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${getRiskLevelColor(item.budgetVariance.riskLevel)}`}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                              <div>
                                ...
                                <div className="space-y-1">
                                  ...
                                  <div className="flex justify-between">
                                    <span>Overhead ({item.costBreakdown.overheadPercentage}%):</span>
                              ...
                              <div>
                                ...
                                <div className="space-y-1">
                                  {item.regionalFactors.filter(f => f.isActive).map(factor => (
                                    <div key={factor.id} className="flex justify-between">
              ...
              <tfoot>
                <tr className="font-bold bg-violet-essence/50 text-base">
                  <td colSpan={6} className="p-4 text-right">Total Enhanced Project Budget</td>
                  <td className="p-4 text-right text-persimmon">{formatCurrency(totalEnhancedBudget)}</td>
