...
export class EnterpriseServiceRegistry implements OnModuleInit {
  ...
  async registerService(service: ServiceDefinition): Promise<void> {
    ...
    await this.etcdClient.put(`services/${service.name}/${service.id}`, JSON.stringify(service));
  ...
  private async validateServiceDefinition(service: ServiceDefinition): Promise<void> {
    ...
    try {
      ...
      await this.httpService
        .get(healthUrl, { timeout: service.healthCheck.timeout * 1000 })
        .toPromise();
    } catch (error) {
...
export class SAPIntegrationService {
  ...
  async syncResourceAllocations(projectId: string): Promise<void> {
    ...
    try {
      ...
      await this.errorHandlingService.handleSAPError(error, 'syncResourceAllocations', {
        projectId,
      });
      throw error;
    }
  ...
  async processInvoiceWorkflow(invoiceData: any): Promise<void> {
    try {
      ...
      await this.updateInvoiceWithSAPReference(invoiceData.id, invoiceResult.E_DOCUMENT_NUMBER);
    } catch (error) {
      await this.errorHandlingService.handleSAPError(error, 'processInvoiceWorkflow', {
        invoiceData,
      });
      throw error;
    }
...
export class AutodeskIntegrationService {
  ...
  async getBIMModelViewerToken(modelUrn: string): Promise<string> {
    ...
    try {
      const response = await this.httpService
        .post(`${this.baseURL}/authentication/v1/authenticate`, {
          client_id: this.configService.get('AUTODESK_CLIENT_ID'),
          client_secret: this.configService.get('AUTODESK_CLIENT_SECRET'),
          grant_type: 'client_credentials',
  ...
  async extractBIMData(modelUrn: string): Promise<any> {
    ...
    try {
      ...
      const metadata = await this.httpService
        .get(`${this.baseURL}/modelderivative/v2/designdata/${modelUrn}/metadata`, {
          headers: {
      ...
      // Extract object tree
      const objectTree = await this.httpService
        .get(
      ...
      // Extract properties
      const properties = await this.httpService
        .get(
  ...
  async synchronizeProjectIssues(projectId: string): Promise<ModelIssue[]> {
    ...
    try {
      const response = await this.httpService
        .get(`${this.baseURL}/issues/v1/containers/${projectId}/issues`, {
          headers: {
            Authorization: `Bearer ${this.tokens.access_token}`,
          },
        })
  ...
  private async createBucketIfNotExists(bucketKey: string): Promise<void> {
    try {
      await this.httpService
        .get(`${this.baseURL}/oss/v2/buckets/${bucketKey}/details`, {
          headers: {
            Authorization: `Bearer ${this.tokens.access_token}`,
          },
        })
