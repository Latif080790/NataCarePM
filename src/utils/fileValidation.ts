...
export function isDocumentFile(file: File): boolean {
  return (
    file.type.startsWith('application/') || file.type === 'text/plain' || file.type === 'text/csv'
  );
}
...
export function generateSafeFilename(originalFilename: string): string {
  ...
  const safeName = nameWithoutExt.replace(/[^a-zA-Z0-9_-]/g, '_').substring(0, 200); // Limit length
