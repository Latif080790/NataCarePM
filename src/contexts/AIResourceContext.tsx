...
export const AIResourceProvider: React.FC<AIResourceProviderProps> = ({ children }) => {
  ...
  const loadModelMetadata = useCallback(async (modelId: string): Promise<MLModelMetadata | null> => {
    setState(prev => ({ ...prev, isLoading: true, error: null }));
      ...
      setState(prev => ({
        ...
        recommendations: [
          ...
          ...result.recommendations.map(rec => ({
            ...
            implementationSteps: ['Review recommendation', 'Approve allocation', 'Update schedule'],
  ...
  const getOptimizationResult = useCallback((resultId: string): OptimizationResult | undefined => {
  ...
  const getRecommendations = useCallback((projectId?: string): SchedulingRecommendation[] => {
    if (projectId) {
  ...
  const getAllocations = useCallback((projectId?: string): ResourceAllocation[] => {
    if (projectId) {
  ...
  const getBottlenecks = useCallback((severity?: string): ResourceBottleneck[] => {
    if (severity) {
  ...
  return (
    <AIResourceContext.Provider value={contextValue}>
      {children}
    </AIResourceContext.Provider>
  );
};
