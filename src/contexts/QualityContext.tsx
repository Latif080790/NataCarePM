...
export const QualityProvider: React.FC<QualityProviderProps> = ({ children }) => {
  ...
  const fetchInspections = useCallback(
    async (projectId: string, filterOptions?: QualityFilterOptions) => {
      setInspectionsLoading(true);
      setInspectionsError(null);

      try {
        const fetchedInspections = await qualityService.getInspections(
          projectId,
   ...
   */
  const fetchInspectionById = useCallback(
    async (inspectionId: string): Promise<QualityInspection | null> => {
      try {
  ...
  const updateInspection = useCallback(
    async (inspectionId: string, updates: Partial<QualityInspection>): Promise<void> => {
      setInspectionsLoading(true);
      setInspectionsError(null);
      ...
      try {
        ...
        setInspections((prev) =>
          prev.map((i) => (i.id === inspectionId ? { ...i, ...updates, updatedAt: new Date() } : i))
        );
  ...
  const fetchDefects = useCallback(
    async (projectId: string, filterOptions?: DefectFilterOptions) => {
      setDefectsLoading(true);
      setDefectsError(null);

      try {
        const fetchedDefects = await qualityService.getDefects(
          projectId,
          filterOptions || defectFilters
        );
  ...
  const updateDefect = useCallback(
    async (defectId: string, updates: Partial<Defect>): Promise<void> => {
      setDefectsLoading(true);
      setDefectsError(null);
      ...
      try {
        ...
        setDefects((prev) =>
          prev.map((d) => (d.id === defectId ? { ...d, ...updates, updatedAt: new Date() } : d))
        );
  ...
  const fetchMetrics = useCallback(
    async (projectId: string, periodStart: Date, periodEnd: Date): Promise<void> => {
      ...
      try {
        const metricsData = await qualityService.getQualityMetrics(
          projectId,
          periodStart,
          periodEnd
        );
   ...
   */
  const getInspectionsByStatus = useCallback(
    (status: InspectionStatus): QualityInspection[] => {
   ...
   */
  const getInspectionsByResult = useCallback(
    (result: InspectionResult): QualityInspection[] => {
  ...
  const getCriticalDefects = useCallback((): Defect[] => {
    return defects.filter(
      (d) => d.severity === 'critical' && ['open', 'in_progress'].includes(d.status)
    );
  }, [defects]);
