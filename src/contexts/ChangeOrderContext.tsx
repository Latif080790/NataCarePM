...
export const ChangeOrderProvider: React.FC<ChangeOrderProviderProps> = ({ children }) => {
  ...
  const fetchChangeOrders = useCallback(
    async (projectId: string, filterOptions?: ChangeOrderFilterOptions) => {
      setChangeOrdersLoading(true);
      setChangeOrdersError(null);

      try {
        const fetchedChangeOrders = await changeOrderService.getChangeOrders(
          projectId,
   ...
   */
  const fetchChangeOrderById = useCallback(
    async (changeOrderId: string): Promise<ChangeOrder | null> => {
      try {
  ...
  const updateChangeOrder = useCallback(
    async (changeOrderId: string, updates: Partial<ChangeOrder>): Promise<void> => {
      setChangeOrdersLoading(true);
      setChangeOrdersError(null);
  ...
  const deleteChangeOrder = useCallback(
    async (changeOrderId: string): Promise<void> => {
      setChangeOrdersLoading(true);
      setChangeOrdersError(null);
   ...
   */
  const getChangeOrdersByStatus = useCallback(
    (status: ChangeOrderStatus): ChangeOrder[] => {
   ...
   */
  const getChangeOrdersByType = useCallback(
    (type: ChangeOrderType): ChangeOrder[] => {
   ...
   */
  const refreshChangeOrders = useCallback(
  ...
  return <ChangeOrderContext.Provider value={value}>{children}</ChangeOrderContext.Provider>;
};
