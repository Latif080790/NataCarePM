...
interface ProjectContextType {
  ...
  handleAddPO: (
    po: Omit<PurchaseOrder, 'id' | 'status' | 'items'> & { items: POItem[] }
  ) => Promise<void>;
  handleUpdateAttendance: (
    date: string,
    updates: Map<string, Attendance['status']>
  ) => Promise<void>;
...
export const ProjectProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const fetchInitialData = async () => {
      try {
        ...
        const projectIdToLoad =
          lastProjectId && allProjectIds.includes(lastProjectId)
            ? lastProjectId
  ...
  const handleUpdateProgress = useCallback(
    async (updates: WorkProgress[]) => {
      if (updates.length === 0) return;
      const newReport: Omit<DailyReport, 'id' | 'comments'> = {
        date: getTodayDateString(),
        weather: 'Cerah',
        notes: 'Koreksi progres manual dari sistem.',
        workforce: [],
        workProgress: updates,
        materialsConsumed: [],
        photos: [],
      };
  ...
  const handleAddPO = useCallback(
    async (po: Omit<PurchaseOrder, 'id' | 'status'>) => {
      if (!currentProject?.id || !currentUser) return;
  ...
  const handleUpdateAttendance = useCallback(
    async (date: string, updates: Map<string, Attendance['status']>) => {
      if (!currentProject?.id || !currentUser) return;
      await projectService.updateAttendance(
        currentProject.id,
        date,
  ...
  const handleAddDocument = useCallback(
    async (doc: Omit<Document, 'id' | 'url'>, file: File) => {
      if (!currentProject?.id || !currentUser) return;
