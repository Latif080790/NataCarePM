...
export class ChartOfAccountsService {
  ...
  async createAccount(
    ...
    try {
      ...
      const accountNumber =
        accountData.accountNumber || (await this.generateAccountNumber(accountData.accountType));
  ...
  async getAccountByNumber(accountNumber: string): Promise<ChartOfAccount | null> {
    try {
      ...
      logger.error('getAccountByNumber', 'Failed to get account', error as Error, {
        accountNumber,
      });
      return null;
    }
  ...
  async getAllAccounts(): Promise<ChartOfAccount[]> {
    try {
      const q = query(collection(db, COLLECTIONS.ACCOUNTS), orderBy('accountNumber', 'asc'));
  ...
  async getAccountsByType(accountType: AccountType): Promise<ChartOfAccount[]> {
    try {
      ...
      logger.error('getAccountsByType', 'Failed to get accounts by type', error as Error, {
        accountType,
      });
      return [];
    }
  ...
  async updateAccount(
    ...
    try {
      ...
      if (!existing) {
        throw new APIError(ErrorCodes.NOT_FOUND, `Account ${accountId} not found`, 404);
      }

      // Prevent updating system accounts
      if (existing.isSystemAccount && !updates.isSystemAccount) {
        throw new APIError(ErrorCodes.FORBIDDEN, 'Cannot modify system accounts', 403);
      }
  ...
  async deleteAccount(accountId: string, userId: string): Promise<boolean> {
    try {
      ...
      if (account.isSystemAccount) {
        throw new APIError(ErrorCodes.FORBIDDEN, 'Cannot delete system accounts', 403);
      }
  ...
  async updateAccountBalance(
    ...
    try {
      ...
      const netChange =
        account.normalBalance === 'debit' ? debitAmount - creditAmount : creditAmount - debitAmount;
      ...
      logger.error('updateAccountBalance', 'Failed to update balance', error as Error, {
        accountId,
      });
      throw error;
    }
