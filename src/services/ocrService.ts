...
export class OptimizedOCRService {
  ...
  /**
   * Process document using Backend Cloud Function (Google Cloud Vision/Document AI)
   * Use this for higher accuracy or official auditing.
   */
  async processDocumentWithBackend(file: File, documentId: string): Promise<APIResponse<OCRResult>> {
    return await safeAsync(async () => {
      ...
      this.processingStatus.set(ocrId, { status: 'uploading', progress: 10 });
      ...
      this.processingStatus.set(ocrId, { status: 'processing_backend', progress: 50 });
      ...
      // Adaptation to OCRResult interface
      const ocrResult: OCRResult = {
        id: ocrId,
        documentId,
        extractedText: data.data?.extractedText || '',
        confidence: data.data?.confidence || 0,
        boundingBoxes: data.data?.boundingBoxes || [],
        extractedData: {}, // Backend should extract this too ideally
        processingTime: Date.now() - startTime,
        timestamp: new Date(),
        status: 'completed',
      };

      this.processingStatus.set(ocrId, { status: 'completed', progress: 100, result: ocrResult });
      return ocrResult;

    }, 'optimizedOcrService.processDocumentWithBackend');
  }
...
export const ocrService = optimizedOcrService;
