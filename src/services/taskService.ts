...
export class EnhancedTaskService {
  ...
  // --- Restoration of Standard CRUD Methods for UI Compatibility ---

  static async getTaskById(projectId: string, taskId: string): Promise<APIResponse<Task>> {
    return await safeAsync(async () => {
      const snap = await getDoc(doc(db, `projects/${projectId}/tasks`, taskId));
      if (!snap.exists()) throw new APIError(ErrorCodes.NOT_FOUND, 'Task not found');
      // Basic conversion, assuming dates are strings or handling elsewhere
      return docToType<Task>(snap);
    }, 'getTaskById');
  }

  static async updateTask(projectId: string, taskId: string, updates: Partial<Task>, user: User): Promise<APIResponse<void>> {
    return await safeAsync(async () => {
      // validateProjectId(projectId);
      // validateTaskId(taskId);
      const updatePayload: any = { ...updates, updatedAt: serverTimestamp() };
      if (user && user.id) updatePayload.updatedBy = user.id;

      await updateDoc(doc(db, `projects/${projectId}/tasks`, taskId), updatePayload);
    ...
    }, 'updateTask', user?.id || 'system');
  }

  static async deleteTask(projectId: string, taskId: string, user: User): Promise<APIResponse<void>> {
    return await safeAsync(async () => {
      await deleteDoc(doc(db, `projects/${projectId}/tasks`, taskId));
    }, 'deleteTask', user?.id || 'system');
  }

  static async addSubtask(projectId: string, taskId: string, subtask: any): Promise<APIResponse<void>> {
    return await safeAsync(async () => {
      const ref = doc(db, `projects/${projectId}/tasks`, taskId);
      const snap = await getDoc(ref);
      if (!snap.exists()) throw new APIError(ErrorCodes.NOT_FOUND, 'Task not found');
      const task = snap.data() as Task;
      const newSubtask = { id: subtask.id || Date.now().toString(), ...subtask };
      const newSubtasks = [...(task.subtasks || []), newSubtask];
      await updateDoc(ref, { subtasks: newSubtasks });
    }, 'addSubtask');
  }

  static async updateSubtask(projectId: string, taskId: string, subtaskId: string, updates: any): Promise<APIResponse<void>> {
    return await safeAsync(async () => {
      const ref = doc(db, `projects/${projectId}/tasks`, taskId);
      const snap = await getDoc(ref);
      if (!snap.exists()) throw new APIError(ErrorCodes.NOT_FOUND, 'Task not found');
      const task = snap.data() as Task;
      const newSubtasks = (task.subtasks || []).map((st: any) => st.id === subtaskId ? { ...st, ...updates } : st);
      await updateDoc(ref, { subtasks: newSubtasks });
    }, 'updateSubtask');
  }

  static async deleteSubtask(projectId: string, taskId: string, subtaskId: string): Promise<APIResponse<void>> {
    return await safeAsync(async () => {
      const ref = doc(db, `projects/${projectId}/tasks`, taskId);
      const snap = await getDoc(ref);
      if (!snap.exists()) throw new APIError(ErrorCodes.NOT_FOUND, 'Task not found');
      const task = snap.data() as Task;
      const newSubtasks = (task.subtasks || []).filter((st: any) => st.id !== subtaskId);
      await updateDoc(ref, { subtasks: newSubtasks });
    }, 'deleteSubtask');
  }

  static async addComment(projectId: string, taskId: string, content: string, user: User): Promise<APIResponse<void>> {
    return await safeAsync(async () => {
    ...
    }, 'addComment', user?.id || 'system');
  }

  static streamTaskComments(projectId: string, taskId: string, callback: (comments: any[]) => void): () => void {
    const q = query(
...
export const taskService = EnhancedTaskService;
