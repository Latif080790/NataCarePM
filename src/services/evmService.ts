...
export class EVMService {
   ...
   */
  static calculateEVMMetrics(projectId: string, input: EVMCalculationInput): EVMMetrics {
    const { tasks, rabItems, actualCosts, currentDate, projectStartDate, budgetAtCompletion } =
      input;

    // Calculate task progress and values
    const taskProgressData = this.calculateTaskProgress(
      tasks,
      rabItems,
      actualCosts,
      currentDate,
  ...
  private static calculateTimeMetrics(
    ...
    const timeVariance =
      (actualTimeProgress - expectedProgress) * (plannedDuration / (1000 * 60 * 60 * 24));
  ...
  private static calculateHealthScore(
    ...
    const progressScore =
      budgetAtCompletion > 0 ? Math.min(plannedValue / budgetAtCompletion, 1.0) : 0;
  ...
  static forecastProjectCompletion(
    ...
    const performanceStability = Math.min(
      evmMetrics.costPerformanceIndex,
      evmMetrics.schedulePerformanceIndex
    );
