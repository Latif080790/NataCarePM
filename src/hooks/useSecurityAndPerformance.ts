...
export const useSecurityManager = () => {
  ...
  const logSecurityEvent = useCallback(
    (type: SecurityMetrics['securityEvents'][0]['type'], details: any, userId?: string) => {
      const event = {
        type,
        userId,
        timestamp: new Date(),
...
export const usePerformanceMonitor = () => {
  const [performanceConfig, setPerformanceConfig] =
    useState<PerformanceConfig>(defaultPerformanceConfig);
  ...
  const optimizeImage = useCallback(
    (file: File, maxWidth: number = 1920, maxHeight: number = 1080): Promise<Blob> => {
  ...
  const generatePerformanceReport = useCallback(() => {
    const report = {
      averagePageLoadTime: Object.entries(performanceMetrics.pageLoadTimes).reduce(
        (acc, [page, times]) => {
      ...
      averageApiResponseTime: Object.entries(performanceMetrics.apiResponseTimes).reduce(
        (acc, [endpoint, times]) => {
      ...
      memoryUsage: {
        ...
        average:
          performanceMetrics.memoryUsage.reduce((sum, usage) => sum + usage, 0) /
            performanceMetrics.memoryUsage.length || 0,
        peak: Math.max(...performanceMetrics.memoryUsage) || 0,
      },

      errorRates: Object.entries(performanceMetrics.errorCounts).reduce(
        (acc, [endpoint, count]) => {
