...
import { EnhancedRabItem, RegionalPriceFactor } from '@/types';
...
export const RegionalPriceAdjustment: React.FC<RegionalPriceAdjustmentProps> = ({
  ...
  const indonesianRegions = [
    ...
    { name: 'Bandung', factor: 0.9, description: 'Major city in West Java' },
    { name: 'Medan', factor: 0.85, description: 'Major city in North Sumatra' },
    { name: 'Makassar', factor: 1.1, description: 'Major city in South Sulawesi' },
    { name: 'Balikpapan', factor: 1.25, description: 'Industrial city in East Kalimantan' },
    { name: 'Jayapura', factor: 1.4, description: 'Capital of Papua' },
    ...
    { name: 'Remote Areas', factor: 1.5, description: 'Remote/difficult access areas' },
  ...
  const regionalImpact = useMemo(() => {
    ...
    const totalAdjustment = activeFactors.reduce(
      (sum, factor) => sum + (factor.adjustmentFactor - 1),
      0
    );

    const avgAdjustment = activeFactors.length > 0 ? totalAdjustment / activeFactors.length : 0;

    const regionDistribution = activeFactors.reduce(
      (acc, factor) => {
  ...
  const filteredItems = useMemo(() => {
    ...
    return rabItems.filter((item) =>
      item.regionalFactors.some((factor) => factor.region === selectedRegion && factor.isActive)
    );
  ...
  const calculateAdjustedPrice = (item: EnhancedRabItem, targetRegion?: string): number => {
    ...
    const applicableFactors = item.regionalFactors.filter(
      (factor) =>
        factor.isActive &&
  ...
  const handleAddFactor = () => {
    ...
    const factor: RegionalPriceFactor = {
      ...
      expiryDate:
        newFactor.expiryDate || new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString(),
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p
                  className={`text-2xl font-bold ${getAdjustmentColor(1 + regionalImpact.avgAdjustment / 100)}`}
                >
                  {regionalImpact.avgAdjustment >= 0 ? '+' : ''}
                  {regionalImpact.avgAdjustment.toFixed(1)}%
                </p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-500">{regionalImpact.totalFactors}</p>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {indonesianRegions.map((region) => (
              ...
              >
                ...
                <p className="text-xs text-persimmon mt-1">Factor: {region.factor}x</p>
              </div>
            ))}
          </div>
        </CardContent>
                ...
                {filteredItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-3 text-right">
                        <div>
                          <p className="font-medium text-orange-600">{formatCurrency(maxDiff)}</p>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setNewFactor({ ...newFactor, adjustmentFactor: Number(e.target.value) })
                  }
                  min="0.1"
                  max="3.0"
                />
            ...
            {newFactor.adjustmentFactor && (
              <div className="p-3 bg-violet-essence/20 rounded-lg">
                ...
                <p className="text-xs text-palladium">
                  ...
                  {formatCurrency(1000000 * (newFactor.adjustmentFactor || 1))}
                </p>
              </div>
            )}
