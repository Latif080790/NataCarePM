      ...
      {metrics && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">{Math.round(metrics.cpu)}%</div>
            <div className="text-sm text-gray-600">CPU Usage</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-green-600">{Math.round(metrics.memory)}MB</div>
...
const ErrorLogModal: React.FC<ErrorLogModalProps> = ({ isOpen, onClose, errors, onResolve }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Error Logs" size="lg">
      <div className="space-y-4 max-h-96 overflow-y-auto">
        {errors.length === 0 ? (
          <div className="text-center py-8 text-gray-500">✅ No unresolved errors</div>
        ) : (
          errors.map((error) => (
            <div key={error.errorId} className="border rounded-lg p-4 bg-gray-50">
              <div className="flex items-start justify-between mb-2">
                <span
                  className={`px-2 py-1 rounded text-sm font-medium ${getSeverityColor(error.severity)}`}
                >
                  {error.severity.toUpperCase()}
                </span>
                <span className="text-sm text-gray-500">{error.timestamp.toLocaleString()}</span>
              </div>
              ...
              {error.userName && (
                <p className="text-sm text-gray-600 mb-2">User: {error.userName}</p>
              )}
...
export const MonitoringDashboard: React.FC<MonitoringDashboardProps> = ({
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      {health && (
        <HealthStatus status={health.status} issues={health.issues} metrics={health.metrics} />
      )}
          ...
          {metricsChartData.length > 0 ? (
            <div className="h-64">
              <svg width="100%" height="100%" viewBox="0 0 800 200">
                ...
                <text x="50" y="170" className="text-xs fill-gray-600">
                  CPU Usage Trend
                </text>
              </svg>
            </div>
