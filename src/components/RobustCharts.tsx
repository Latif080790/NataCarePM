    ...
    <div className="animate-pulse" style={{ height }}>
      <div className="bg-gray-200 rounded-lg h-full flex items-center justify-center">
        <div className="space-y-3 w-full max-w-md px-4">
          ...
          {type === 'gauge' && <div className="w-32 h-32 bg-gray-300 rounded-full mx-auto"></div>}
        </div>
      </div>
    </div>
  );
        ...
        <div className="flex flex-col items-center justify-center h-64 bg-red-50 border border-red-200 rounded-lg">
          ...
          {this.state.error && (
            <details className="mt-2 text-xs text-red-500">
              ...
              <pre className="mt-1 p-2 bg-red-100 rounded">{this.state.error.message}</pre>
            </details>
          )}
          <Button
            variant="outline"
  ...
  const processedData = useMemo(() => {
    ...
    const scaledSeries = data.map((series, index) => ({
      ...
      scaledData: series.data.map((point) => ({
        ...point,
        x:
          ((point.timestamp.getTime() - xRange[0]) / (xRange[1] - xRange[0])) *
            (dimensions.width - 100) +
          50,
        y:
          dimensions.height -
          50 -
          ((point.value - yRange[0]) / (yRange[1] - yRange[0])) * (dimensions.height - 100),
      })),
    }));
          ...
          >
            ...
            <g className="text-xs fill-gray-600">
              ...
              {[...Array(5)].map((_, i) => {
                const y = 50 + (i * (dimensions.height - 100)) / 4;
                const value =
                  processedData.yRange[1] -
                  (i * (processedData.yRange[1] - processedData.yRange[0])) / 4;
                return (
          ...
          <div className="flex flex-wrap gap-4 mt-4 justify-center">
            {processedData.series.map((series, index) => (
              <div key={`legend-${index}`} className="flex items-center gap-2">
                <div className="w-4 h-4 rounded" style={{ backgroundColor: series.color }}></div>
            ...
            <div className="space-y-2 h-full flex flex-col justify-around">
              {data.map((item, index) => (
                <div key={index} className="flex items-center">
                  <div className="w-20 text-xs text-right pr-2 truncate">{item.name}</div>
            ...
            <div className="flex items-end justify-around h-full space-x-1">
              {data.map((item, index) => (
                <div key={index} className="flex flex-col items-center flex-1">
                  ...
                  <div className="text-xs text-center mt-2 max-w-full truncate">{item.name}</div>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="relative">
          ...
          {showValue && (
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <div className="text-3xl font-bold text-gray-800">{Math.round(normalizedValue)}</div>
