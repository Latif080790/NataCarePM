...
const LineChartComponent: React.FC<LineChartProps> = ({ data, width, height }) => {
  ...
  if (!data.planned.length && !data.actual.length) {
    return (
      <div
        style={{ width, height }}
        className="flex items-center justify-center text-palladium glass rounded-2xl border border-violet-essence/20"
      >
  ...
  const createPath = (points: Point[]) => {
    ...
    return points
      .map((p, i) => `${i === 0 ? 'M' : 'L'} ${xScale(p.day)} ${yScale(p.cost)}`)
      .join(' ');
  };
  ...
  return (
    <div className="relative">
      <svg
        ...
        className="bg-gradient-to-br from-brilliance/20 to-violet-essence-50/30 rounded-2xl"
      >
        <defs>
        ...
        >
          ...
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs">
              Rencana: <span className="font-semibold">{formatCurrency(tooltip.planned)}</span>
            </span>
          </div>
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded-full bg-precious-persimmon"></span>
            <span className="text-xs">
              Aktual: <span className="font-semibold">{formatCurrency(tooltip.actual)}</span>
            </span>
          </div>
        </div>
      )}
