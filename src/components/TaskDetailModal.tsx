...
export default function TaskDetailModal({
  ...
  useEffect(() => {
    ...
    const unsubscribe = taskService.streamTaskComments(currentProject.id, task.id, setComments);
  ...
  const handleToggleSubtask = async (subtaskId: string, completed: boolean) => {
    ...
    try {
      await taskService.updateSubtask(currentProject.id, task.id, subtaskId, { completed });
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span
                className={`px-2 py-1 rounded-full text-xs font-semibold ${getStatusColor(taskData.status)}`}
              >
                {taskData.status}
              </span>
              <span
                className={`px-2 py-1 rounded-full text-xs font-semibold ${getPriorityColor(taskData.priority)}`}
              >
                {taskData.priority}
              </span>
              {isOverdue && (
        ...
        <div className="bg-violet-essence/20 p-4 rounded-lg">
          ...
          <p className="text-xs text-palladium mt-1">
            {taskData.subtasks.filter((st) => st.completed).length} dari {taskData.subtasks.length}{' '}
            subtasks selesai
          </p>
        </div>
        ...
        <div>
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {taskData.subtasks.map((subtask) => (
              <div
                key={subtask.id}
                className="flex items-center gap-2 p-2 border border-violet-essence rounded"
              >
                <input
                  type="checkbox"
                ...
                />
                <span
                  className={`flex-1 ${subtask.completed ? 'line-through text-palladium' : ''}`}
                >
                  {subtask.title}
                </span>
                {canEdit && (
              ...
              <div key={comment.id} className="flex gap-3 p-3 bg-gray-50 rounded">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs text-palladium">{formatDate(comment.timestamp)}</span>
                  </div>
                  <div
