...
export const PriceEscalationManager: React.FC<PriceEscalationManagerProps> = ({
    ...
    const handleCalculateEscalation = (item: EnhancedRabItem) => {
        ...
        const newEscalation: PriceEscalation = {
            ...
            projectionDate: new Date(Date.now() + projectionMonths * 30 * 24 * 60 * 60 * 1000).toISOString(),
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-2xl font-bold text-night-black">
                                    {(rabItems.reduce((sum, item) => sum + item.escalationRate, 0) / rabItems.length).toFixed(1)}%
                                </p>
                            </div>
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-2xl font-bold text-persimmon">
                                    {formatCurrency(
                                        rabItems.reduce((sum, item) => 
                                            sum + (calculateProjectedPrice(item, 12) - item.hargaSatuan) * item.volume, 0
                                        )
                                    )}
                                </p>
                            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {defaultMarketFactors.map(factor => (
                            <div key={factor.id} className="p-4 bg-violet-essence/20 rounded-lg">
                                ...
                                <p className="text-xs text-palladium">Weight: {(factor.weight * 100).toFixed(0)}%</p>
                            </div>
                        ))}
                    </div>
                </CardContent>
                                ...
                                {rabItems.map(item => {
                                    ...
                                    return (
                                        <tr key={item.id} className="border-b border-violet-essence hover:bg-violet-essence/30">
                                            ...
                                            <td className="p-3 text-right">
                                                <div>
                                                    <p className="font-medium text-persimmon">{formatCurrency(projectedPrice)}</p>
                                            ...
                                            <td className="p-3 text-center">
                                                <span className={`px-2 py-1 rounded-full text-xs font-medium ${riskLevel.color} bg-opacity-20`}>
                                                    {riskLevel.level}
                                                </span>
                                            </td>
                                            <td className="p-3 text-center">
                            ...
                            <Input
                                ...
                                onChange={(e) => {
                                    ...
                                    setEscalationData({
                                        ...
                                        projectionDate: new Date(Date.now() + months * 30 * 24 * 60 * 60 * 1000).toISOString()
                                    });
                                }}
                                min="1"
                                max="60"
                        ...
                        <div>
                            ...
                            <div className="space-y-2">
                                {escalationData.marketFactors.map(factor => (
                                    <div key={factor.id} className="flex justify-between items-center p-2 bg-violet-essence/20 rounded">
                                        <span className="text-sm">{factor.factor}</span>
                                        <div className="text-right">
                                            <span className="text-sm font-medium">{factor.currentValue}</span>
                                            <span className="text-xs text-palladium ml-2">({(factor.weight * 100).toFixed(0)}%)</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
