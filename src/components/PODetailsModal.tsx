    ...
    <Modal isOpen={!!po} onClose={onClose} title={`Detail PO: ${po.prNumber}`}>
      <div className="space-y-4 text-sm">
        <div className="grid grid-cols-2 gap-2 p-2 bg-violet-essence/30 rounded-md">
          <strong className="text-palladium">Nomor PR:</strong>
          <span>{po.prNumber}</span>
          <strong className="text-palladium">Status:</strong>
          <span>{po.status}</span>
          <strong className="text-palladium">Pemohon:</strong>
          <span>{po.requester}</span>
          <strong className="text-palladium">Tanggal:</strong>
          <span>{formatDate(po.requestDate)}</span>
        </div>
