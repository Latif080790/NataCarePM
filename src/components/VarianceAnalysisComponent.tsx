...
import { TrendingUp, TrendingDown, AlertCircle, Clock, DollarSign, Target } from 'lucide-react';
import { EnhancedRabItem } from '@/types';
...
export const VarianceAnalysisComponent: React.FC<VarianceAnalysisComponentProps> = React.memo(
  ({ rabItems }) => {
    ...
    const overallMetrics = useMemo(() => {
      const totalBudgeted = rabItems.reduce(
        (sum, item) => sum + item.budgetVariance.budgetedCost * item.volume,
        0
      );
      const totalActual = rabItems.reduce(
        (sum, item) => sum + item.budgetVariance.actualCost * item.volume,
        0
      );
      const totalVariance = totalActual - totalBudgeted;
      const variancePercentage = totalBudgeted > 0 ? (totalVariance / totalBudgeted) * 100 : 0;

      const itemsOverBudget = rabItems.filter(
        (item) => item.budgetVariance.costVariance > 0
      ).length;
      const itemsBehindSchedule = rabItems.filter(
        (item) => item.budgetVariance.timeVariance > 0
      ).length;
      const criticalItems = rabItems.filter(
        (item) => item.budgetVariance.riskLevel === 'critical'
      ).length;
    ...
    const sortedItems = useMemo(() => {
      return [...rabItems].sort((a, b) => {
        switch (sortBy) {
          case 'variance':
            return (
              Math.abs(b.budgetVariance.costVariancePercentage) -
              Math.abs(a.budgetVariance.costVariancePercentage)
            );
          case 'risk':
          ...
          case 'impact':
            return (
              Math.abs(b.budgetVariance.costVariance * b.volume) -
              Math.abs(a.budgetVariance.costVariance * a.volume)
            );
          default:
            return 0;
        }
      ...
      <div className="space-y-6">
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p
                    className={`text-2xl font-bold ${getPerformanceColor(overallMetrics.variancePercentage)}`}
                  >
                    {overallMetrics.variancePercentage >= 0 ? '+' : ''}
                    {overallMetrics.variancePercentage.toFixed(1)}%
                  </p>
                  <p className="text-xs text-palladium">
                    {formatCurrency(overallMetrics.totalVariance)}
                  </p>
                </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-palladium">of {overallMetrics.totalItems} items</p>
                </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-palladium">items delayed</p>
                </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-red-600">{overallMetrics.criticalItems}</p>
                  <p className="text-xs text-palladium">need attention</p>
                </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Perbandingan kumulatif biaya yang direncanakan dengan aktual
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-64">
              <LineChart data={generateTrendData()} width={800} height={250} />
            </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="p-3 text-right">
                          <div>
                            <p
                              className={`font-medium ${getPerformanceColor(variance.costVariancePercentage)}`}
                            >
                              {variance.costVariancePercentage >= 0 ? '+' : ''}
                              {variance.costVariancePercentage.toFixed(1)}%
                            </p>
                        ...
                        <td className="p-3 text-right">
                          <div>
                            <p
                              className={`font-medium ${variance.timeVariance > 0 ? 'text-red-500' : variance.timeVariance < 0 ? 'text-green-500' : 'text-blue-500'}`}
                            >
                              {variance.timeVariance >= 0 ? '+' : ''}
                              {variance.timeVariance} days
                            </p>
                            <p className="text-xs text-palladium">
                              {variance.timeVariancePercentage >= 0 ? '+' : ''}
                              {variance.timeVariancePercentage.toFixed(1)}%
                            </p>
                          </div>
                        </td>
                        <td className="p-3 text-center">
                          <div className="flex flex-col items-center">
                            ...
                            <span
                              className={`text-xs font-medium ${variance.performanceIndex <= 1 ? 'text-green-600' : 'text-red-600'}`}
                            >
                        ...
                        <td className="p-3 text-center">
                          <span
                            className={`px-2 py-1 rounded-full text-xs font-medium ${getRiskColor(variance.riskLevel)}`}
                          >
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span
                    className={`font-bold ${overallMetrics.variancePercentage <= 0 ? 'text-green-600' : 'text-red-600'}`}
                  >
            ...
            <CardContent>
              <div className="space-y-3">
                {['low', 'medium', 'high', 'critical'].map((risk) => {
                  const count = rabItems.filter(
                    (item) => item.budgetVariance.riskLevel === risk
                  ).length;
