...
async function performHealthCheck() {
    ...
    try {
        ...
        console.log(`  Document Creation: ${createTime}ms ${createTime < 1000 ? '✅' : createTime < 3000 ? '⚠️' : '❌'}`);
        
        // Test retrieval performance
        const retrieveStart = Date.now();
        await mockServices.intelligentDocumentService.getDocument(document.id);
        const retrieveTime = Date.now() - retrieveStart;
        console.log(`  Document Retrieval: ${retrieveTime}ms ${retrieveTime < 500 ? '✅' : retrieveTime < 1500 ? '⚠️' : '❌'}`);
        
        // Test search performance
        const searchStart = Date.now();
        await mockServices.intelligentDocumentService.searchDocuments('Health Check', 'health_check_project');
        const searchTime = Date.now() - searchStart;
        console.log(`  Document Search: ${searchTime}ms ${searchTime < 800 ? '✅' : searchTime < 2000 ? '⚠️' : '❌'}`);
        
        // Get system metrics
        const metrics = await mockServices.intelligentDocumentService.getProcessingMetrics();
        console.log(`  Processing Success Rate: ${(metrics.successRate * 100).toFixed(1)}% ${metrics.successRate > 0.9 ? '✅' : metrics.successRate > 0.8 ? '⚠️' : '❌'}`);
        console.log(`  OCR Accuracy: ${(metrics.ocrAccuracy * 100).toFixed(1)}% ${metrics.ocrAccuracy > 0.9 ? '✅' : metrics.ocrAccuracy > 0.8 ? '⚠️' : '❌'}`);
    ...
    try {
        ...
        const missingFields = requiredFields.filter(field => !(field in integrityDoc) || integrityDoc[field] === undefined);
        
        console.log(`  Schema Validation: ${missingFields.length === 0 ? '✅ VALID' : `❌ MISSING: ${missingFields.join(', ')}`}`);
        ...
        // Cleanup
        await mockServices.intelligentDocumentService.deleteDocument(integrityDoc.id, 'health_check_user');
    ...
    try {
        ...
        console.log(`  Average Processing Time: ${analytics.processingMetrics.averageProcessingTime}ms`);
    ...
    const statusIcon = healthMetrics.systemStatus === 'HEALTHY' ? '✅' : healthMetrics.systemStatus === 'DEGRADED' ? '⚠️' : '❌';
