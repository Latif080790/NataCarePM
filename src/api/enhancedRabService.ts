...
export class EnhancedRabService {
  ...
  static applyRegionalAdjustments(
    ...
    const activeFactors = regionalFactors.filter(
      (factor) =>
        factor.isActive &&
  ...
  static analyzePriceHistory(priceHistory: PriceHistory[]): {
    ...
    const avgReliability =
      priceHistory.reduce((sum, h) => sum + h.reliability, 0) / priceHistory.length;
   ...
   */
  static generateRiskAssessment(enhancedItem: EnhancedRabItem): ProjectRiskProfile {
    const riskFactors = [];
    ...
    return {
      ...
      costRisk: Math.min(
        50,
        enhancedItem.escalationRate + (enhancedItem.budgetVariance.costVariancePercentage || 0)
      ),
