...
const validateTaskId = (taskId: string): void => { if (!validators.isValidId(taskId)) throw new APIError(ErrorCodes.INVALID_INPUT, 'Invalid task ID', 400, { taskId }); };
const validateUserId = (userId: string): void => { if (!validators.isValidId(userId)) throw new APIError(ErrorCodes.INVALID_INPUT, 'Invalid user ID', 400, { userId }); };
...
export const taskService = {
	...
	streamTasksByProject: (projectId: string, callback: (tasks: Task[]) => void, errorCallback?: (error: Error) => void): Unsubscribe => {
		try {
			validateProjectId(projectId);
			const q = query(collection(db, `projects/${projectId}/tasks`), orderBy('createdAt', 'desc'));
	...
	getTaskById: async (projectId: string, taskId: string): Promise<APIResponse<Task>> => {
		return await safeAsync(async () => {
			...
			if (!snap.exists()) throw new APIError(ErrorCodes.NOT_FOUND, 'Task not found', 404, { projectId, taskId });
			return docToType<Task>(snap);
		}, 'taskService.getTaskById');
	},
	filterTasks: async (projectId: string, filters: { status?: string; priority?: string; assignedTo?: string; search?: string; }): Promise<APIResponse<Task[]>> => {
		return await safeAsync(async () => {
			validateProjectId(projectId);
			...
			if (filters.assignedTo) q = query(q, where('assignedTo', 'array-contains', filters.assignedTo));
			const snapshot = await withRetry(() => getDocs(q), { maxAttempts: 3 });
	...
	createTask: async (projectId: string, taskData: Omit<Task, 'id' | 'createdAt' | 'updatedAt'>, user: User): Promise<APIResponse<string>> => {
		return await safeAsync(async () => {
			...
			if (!titleValidation.valid) throw new APIError(ErrorCodes.VALIDATION_FAILED, 'Invalid task title', 400, { errors: titleValidation.errors });
			const docRef = await withRetry(() => addDoc(collection(db, `projects/${projectId}/tasks`), { ...taskData, createdAt: serverTimestamp(), updatedAt: serverTimestamp() }), { maxAttempts: 3 });
			try { await projectService.addAuditLog(projectId, user, `Membuat task baru: "${taskData.title}"`); } catch {}
			return docRef.id;
		}, 'taskService.createTask', user.id);
	},
	updateTask: async (projectId: string, taskId: string, updates: Partial<Task>, user: User): Promise<APIResponse<void>> => {
		return await safeAsync(async () => {
			validateProjectId(projectId);
			validateTaskId(taskId);
			validateUserId(user.id);
			...
			await withRetry(() => updateDoc(taskRef, { ...updates, updatedAt: serverTimestamp() }), { maxAttempts: 3 });
			try { await projectService.addAuditLog(projectId, user, `Memperbarui task #${taskId}`); } catch {}
			return;
	...
	deleteTask: async (projectId: string, taskId: string, user: User): Promise<APIResponse<void>> => {
		return await safeAsync(async () => {
			...
			if (!taskDoc.exists()) throw new APIError(ErrorCodes.NOT_FOUND, 'Task not found', 404, { projectId, taskId });
			await withRetry(() => deleteDoc(taskRef), { maxAttempts: 3 });
			try { await projectService.addAuditLog(projectId, user, `Menghapus task: "${taskDoc.data()?.title || 'Unknown'}"`); } catch {}
			return;
		}, 'taskService.deleteTask', user.id);
		},
	  // --- Subtask CRUD ---
	  addSubtask: async (projectId: string, taskId: string, subtaskData: Omit<Subtask, 'id'>): Promise<APIResponse<string>> => {
		return await safeAsync(async () => {
			validateProjectId(projectId);
			validateTaskId(taskId);
			const taskRef = doc(db, `projects/${projectId}/tasks`, taskId);
			const taskSnap = await withRetry(() => getDoc(taskRef), { maxAttempts: 2 });
			if (!taskSnap.exists()) throw new APIError(ErrorCodes.NOT_FOUND, 'Task not found', 404, { projectId, taskId });
			const task = docToType<Task>(taskSnap);
			const newSubtask: Subtask = { ...subtaskData, id: generateSubtaskId(), completed: false };
			await withRetry(() => updateDoc(taskRef, { subtasks: [...(task.subtasks || []), newSubtask], updatedAt: serverTimestamp() }), { maxAttempts: 3 });
			return newSubtask.id;
		}, 'taskService.addSubtask');
	},
	updateSubtask: async (projectId: string, taskId: string, subtaskId: string, updates: Partial<Subtask>): Promise<APIResponse<void>> => {
		return await safeAsync(async () => {
			validateProjectId(projectId);
			validateTaskId(taskId);
			const taskRef = doc(db, `projects/${projectId}/tasks`, taskId);
			...
			await withRetry(() => updateDoc(taskRef, { subtasks: updatedSubtasks, updatedAt: serverTimestamp() }), { maxAttempts: 3 });
			return;
		}, 'taskService.updateSubtask');
	},
	deleteSubtask: async (projectId: string, taskId: string, subtaskId: string): Promise<APIResponse<void>> => {
		return await safeAsync(async () => {
			validateProjectId(projectId);
			validateTaskId(taskId);
			const taskRef = doc(db, `projects/${projectId}/tasks`, taskId);
	...
	// --- Task Comments ---
	streamTaskComments: (projectId: string, taskId: string, callback: (comments: TaskComment[]) => void, errorCallback?: (error: Error) => void): Unsubscribe => {
		try {
			validateProjectId(projectId);
			validateTaskId(taskId);
			const q = query(collection(db, `projects/${projectId}/tasks/${taskId}/comments`), orderBy('timestamp', 'asc'));
			return onSnapshot(q, (qs) => {
				try {
	...
	addTaskComment: async (projectId: string, taskId: string, comment: Omit<TaskComment, 'id' | 'timestamp'>): Promise<APIResponse<string>> => {
		return await safeAsync(async () => {
			validateProjectId(projectId);
			validateTaskId(taskId);
			const docRef = await withRetry(() => addDoc(collection(db, `projects/${projectId}/tasks/${taskId}/comments`), { ...comment, timestamp: new Date().toISOString() }), { maxAttempts: 3 });
			return docRef.id;
		}, 'taskService.addTaskComment');
	},
	deleteTaskComment: async (projectId: string, taskId: string, commentId: string): Promise<APIResponse<void>> => {
		return await safeAsync(async () => {
			validateProjectId(projectId);
			validateTaskId(taskId);
	...
	recalculateTaskProgress: async (projectId: string, taskId: string): Promise<APIResponse<void>> => {
		return await safeAsync(async () => {
			...
			let progress = 0;
			if (subtasks.length === 0) progress = task.status === 'done' ? 100 : task.status === 'in-progress' ? 50 : 0;
			else {
				const completed = subtasks.filter(st => st.completed).length;
				progress = Math.round((completed / subtasks.length) * 100);
			}
			await withRetry(() => updateDoc(taskRef, { progress, updatedAt: serverTimestamp() }), { maxAttempts: 2 });
			return;
