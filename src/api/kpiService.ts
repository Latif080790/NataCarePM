...
export class KPIService {
  ...
  static calculateKPIMetrics(input: KPICalculationInput): KPIMetrics {
    ...
    // Financial KPIs
    const financialKPIs = this.calculateFinancialKPIs(actualCosts, budgetAtCompletion, evmMetrics);
  ...
  private static calculateFinancialKPIs(
    ...
    const budgetUtilization =
      budgetAtCompletion > 0 ? (totalActualCost / budgetAtCompletion) * 100 : 0;
    ...
    const returnOnInvestment =
      totalActualCost > 0 ? ((estimatedProjectValue - totalActualCost) / totalActualCost) * 100 : 0;
  ...
  private static calculateScheduleKPIs(tasks: Task[], evmMetrics?: EVMMetrics) {
    ...
    const milestoneAdherence =
      milestoneTasks.length > 0 ? (completedMilestones / milestoneTasks.length) * 100 : 100;
  ...
  private static calculateResourceKPIs(
    ...
    const resourceUtilization = allocatedHours > 0 ? (actualHours / allocatedHours) * 100 : 0;

    const productivityIndex = productivity || 100;

    // Team efficiency based on task completion vs time
    const teamEfficiency = tasks ? this.calculateTeamEfficiency(tasks, actualHours) : 90;
  ...
  private static calculateTeamEfficiency(tasks: Task[], actualHours: number): number {
    const totalEstimatedHours = tasks.reduce((sum, task) => {
      ...
      return (
        sum +
        (((task.endDate ? new Date(task.endDate).getTime() : Date.now()) -
          (task.startDate ? new Date(task.startDate).getTime() : Date.now())) /
          (1000 * 60 * 60 * 24)) *
          8
      );
    }, 0);
  ...
  private static determinePerformanceTrend(metrics: any): 'Improving' | 'Stable' | 'Declining' {
    ...
    if (metrics.resourceUtilization >= 80 && metrics.resourceUtilization <= 120)
      positiveIndicators++;
    else if (metrics.resourceUtilization < 60 || metrics.resourceUtilization > 140)
      negativeIndicators++;
   ...
   */
  static generateKPITrends(historicalData: { date: Date; metrics: KPIMetrics }[]): {
   ...
   */
  static calculateKPIRatings(metrics: KPIMetrics): {
    ...
    // Schedule KPIs
    ratings.scheduleVariance = this.getRating(
      Math.abs(metrics.scheduleVariancePercentage),
      [2, 5, 10, 20]
    );
    ...
    ratings.contingencyUtilization = this.getRating(
      metrics.contingencyUtilization,
      [15, 25, 35, 50],
      true
    );
