...
export class OCRService {
  ...
  private extractDates(text: string, boundingBoxes: BoundingBox[]): ExtractedDate[] {
    ...
    while ((match = dateRegex.exec(text)) !== null) {
      ...
      if (contextBefore.includes('start') || contextBefore.includes('begin')) {
        ...
        contextBefore.includes('end') ||
        contextBefore.includes('completion') ||
        contextBefore.includes('finish')
      ) {
        type = 'end_date';
  ...
  private extractMaterials(text: string, boundingBoxes: BoundingBox[]): ExtractedMaterial[] {
    const materialRegex =
      /-\s*([A-Za-z\s]+):\s*(\d+(?:\.\d+)?)\s*([A-Za-z³²]+)(?:\s*@\s*\$(\d+(?:\.\d+)?)\/[A-Za-z³²]+)?/g;
  ...
  private extractPersonnel(text: string, boundingBoxes: BoundingBox[]): ExtractedPersonnel[] {
    const personnelRegex =
      /(PROJECT MANAGER|SITE ENGINEER|ARCHITECT|CONTRACTOR|SUPERVISOR):\s*([A-Za-z\s]+)/gi;
  ...
  private extractSpecifications(
    ...
    while ((match = specRegex.exec(text)) !== null) {
      ...
      if (
        !category.toLowerCase().includes('concrete') &&
  ...
  private parseTableLines(lines: string[]): ExtractedTable | null {
    ...
    return {
      ...
      rows,
      confidence: 0.7,
      boundingBox: { x: 0, y: 0, width: 0, height: 0, text: 'Table', confidence: 0.7 },
      category: 'data_table',
    };
  }
  ...
  async getProcessingStatus(
    ocrId: string
  ): Promise<{ status: string; progress: number; result?: OCRResult }> {
