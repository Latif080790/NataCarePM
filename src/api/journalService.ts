...
export class JournalEntriesService {
    ...
    private validateEntry(entry: Partial<JournalEntry>): void {
        ...
        entry.lines.forEach((line, index) => {
            ...
            if (!line.accountId) {
                throw new APIError(
                    ErrorCodes.INVALID_INPUT,
                    `Line ${index + 1}: Account is required`,
                    400
                );
            }
        });
    ...
    async createJournalEntry(
        entryData: Omit<JournalEntry, 'id' | 'entryNumber' | 'createdAt' | 'createdBy' | 'updatedAt' | 'updatedBy' | 'totalDebit' | 'totalCredit' | 'isBalanced'>,
        userId: string
    ...
    async getJournalEntriesByStatus(status: JournalEntryStatus): Promise<JournalEntry[]> {
        try {
            ...
            logger.error('getJournalEntriesByStatus', 'Failed to get entries by status', error as Error, { status });
            return [];
        }
    }
    ...
    async updateJournalEntry(
        ...
        try {
            ...
            logger.error('updateJournalEntry', 'Failed to update journal entry', error as Error, { entryId });
            throw error;
        }
    }
    ...
    async postEntry(entryId: string, userId: string): Promise<JournalEntry | null> {
        try {
            ...
            for (const line of entry.lines) {
                await chartOfAccountsService.updateAccountBalance(
                    line.accountId,
                    line.debit,
                    line.credit
                );
    ...
    async deleteJournalEntry(entryId: string, userId: string): Promise<boolean> {
        try {
            ...
            logger.error('deleteJournalEntry', 'Failed to delete journal entry', error as Error, { entryId });
            throw error;
        }
    }
}
