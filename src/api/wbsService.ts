...
class WBSService {
  ...
  async createWBSElement(
    ...
    try {
      ...
      const variance =
        elementData.budgetAmount - (elementData.actualAmount + elementData.commitments);
      const variancePercentage =
        elementData.budgetAmount > 0 ? (variance / elementData.budgetAmount) * 100 : 0;
  ...
  async deleteWBSElement(
    ...
    try {
      ...
      if (children.length > 0 && !deleteChildren) {
        throw new Error(
          'Cannot delete WBS element with children. Set deleteChildren=true to force delete.'
        );
      }
      ...
      hierarchy.flatList.forEach((element) => {
        if (element.parentId) {
          ...
          if (parent && element.level !== parent.level + 1) {
            errors.push(
              `Level inconsistency: ${element.code} (expected level ${parent.level + 1}, got ${element.level})`
            );
          }
        }
      });
   ...
   */
  private async updateHierarchyLevels(
    elementId: string,
