...
import { 
    ...
    serverTimestamp, orderBy, writeBatch, Unsubscribe, QueryDocumentSnapshot, DocumentData 
    ...
    streamProjectById: (
        ...
        try {
            ...
            return onSnapshot(
                        ...
                        if (docSnapshot.exists()) {
                            ...
                            for (const sc of subCollections) {
                                try {
                                    ...
                                    (projectData as any)[sc] = scSnapshot.docs.map((d: QueryDocumentSnapshot<DocumentData>) => docToType(d));
                                } catch (error) {
                            ...
                            logger.debug('projectService:streamProjectById', 'Project data updated', { projectId });
                        } else {
                            ...
                            logger.error('projectService:streamProjectById', 'Project not found', error, { projectId });
                            errorCallback?.(error);
                        }
                    } catch (error) {
                        logger.error(
                ...
                (error) => {
                    logger.error('projectService:streamProjectById', 'Snapshot listener error', error, { projectId });
                    errorCallback?.(error);
                }
            );
        } catch (error) {
            logger.error('projectService:streamProjectById', 'Failed to setup stream', error instanceof Error ? error : undefined, { projectId });
    ...
    streamNotifications: (
        ...
        try {
            ...
            return onSnapshot(
                ...
                (querySnapshot) => {
                    try {
                        const notifs = querySnapshot.docs.map((d: QueryDocumentSnapshot<DocumentData>) => docToType<Notification>(d));
                        callback(notifs);
                        
                        logger.debug('projectService:streamNotifications', 'Notifications updated', { count: notifs.length });
                    } catch (error) {
                        logger.error('projectService:streamNotifications', 'Error processing notifications', error instanceof Error ? error : undefined);
                        errorCallback?.(error instanceof Error ? error : new Error(String(error)));
                    }
                },
                (error) => {
            ...
            logger.error('projectService:streamNotifications', 'Failed to setup stream', error instanceof Error ? error : undefined);
            return () => {};
        }
    },
            ...
            async () => {
                ...
                const projectsSnapshot = await withRetry(
                    () => getDocs(collection(db, "projects")),
                    {
                        maxAttempts: 3,
                        onRetry: (attempt: number, error: Error) => {
                            logger.warn('projectService:getWorkspaces', `Retry attempt ${attempt}`, { error: error.message });
                        }
                    }
                );

                const projects = projectsSnapshot.docs.map((d: QueryDocumentSnapshot<DocumentData>) => docToType<Project>(d));
    ...
    getWorkers: async (): Promise<APIResponse<Worker[]>> => {
        ...
        const result = await safeAsync(
            async () => {
                ...
                const workers = snapshot.docs.map((d: QueryDocumentSnapshot<DocumentData>) => docToType<Worker>(d));

                logger.info('projectService:getWorkers', 'Workers fetched successfully', { count: workers.length });
    ...
    getUsers: async (): Promise<APIResponse<User[]>> => {
        ...
        const result = await safeAsync(
            async () => {
                ...
                const users = snapshot.docs.map((d: QueryDocumentSnapshot<DocumentData>) => docToType<User>(d));
    ...
    addAuditLog: async (projectId: string, user: User, action: string): Promise<APIResponse<string>> => {
        ...
        const result = await safeAsync(
            async () => {
                ...
                logger.info('projectService:addAuditLog', 'Adding audit log', { projectId, userId: user.id, action });
    ...
    updateAttendance: async (
        ...
        const result = await safeAsync(
            async () => {
                ...
                const existingMap = new Map(existingDocs.docs.map((d: QueryDocumentSnapshot<DocumentData>) => [d.data().workerId as string, d.id]));
            ...
            async () => {
                ...
                notificationIds.forEach(id => {
                    if (validators.isValidId(id)) {
                        ...
                        logger.warn('projectService:markNotificationsAsRead', 'Invalid notification ID skipped', { id });
                    }
                });
