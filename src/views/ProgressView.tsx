    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>
                    Sesuaikan total volume pekerjaan yang telah selesai. Sistem akan otomatis membuat laporan harian baru untuk selisihnya. Gunakan hanya untuk koreksi.
                </CardDescription>
            </CardHeader>
            <CardContent>
                <div className="space-y-4">
                    {itemsWithProgress.map(item => {
                        ...
                        return (
                            <div key={item.id} className="grid grid-cols-1 md:grid-cols-6 gap-4 items-center border-b border-violet-essence pb-2">
                                <div className="md:col-span-3">
                                    <p className="font-medium text-sm">{item.no} {item.uraian}</p>
                                    <p className="text-xs text-palladium">Total Volume: {item.volume.toFixed(2)} {item.satuan}</p>
                                </div>
                                <div className="md:col-span-3 flex items-center gap-4">
                                    ...
                                    />
                                     <span className="text-sm text-palladium">/ {item.volume.toFixed(2)} {item.satuan}</span>
                                    <div className="flex-grow">
                                        <Progress value={progressPercent} />
                                    </div>
                                    <span className="text-sm font-semibold w-12 text-right">{progressPercent.toFixed(1)}%</span>
                                </div>
                            </div>
                        );
                    })}
