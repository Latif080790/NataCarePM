  ...
  const totalImpact = useMemo(() => {
    ...
    return {
      ...
      costPercentage: selectedChangeOrder.budgetImpact 
        ? (selectedChangeOrder.budgetImpact.variance / selectedChangeOrder.budgetImpact.originalBudget) * 100 
        : 0,
      ...
      schedulePercentage: selectedChangeOrder.scheduleImpactDetail
        ? (selectedChangeOrder.scheduleImpact / 
           ((selectedChangeOrder.scheduleImpactDetail.newEndDate.getTime() - 
             selectedChangeOrder.scheduleImpactDetail.originalEndDate.getTime()) / (1000 * 60 * 60 * 24))) * 100
        : 0,
    };
  }, [selectedChangeOrder]);
          ...
          <>
                    ...
                    <div className="ml-5 w-0 flex-1">
                      <dl>
                        ...
                        <dd className={`text-lg font-semibold ${
                          ...
                          {selectedChangeOrder.costImpact >= 0 ? '+' : ''}{formatCurrency(selectedChangeOrder.costImpact)}
                        </dd>
                      </dl>
                    </div>
                  </div>
            ...
            <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-6 mb-6">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3">
                    <div
                      ...
                      style={{ width: `${Math.min(Math.abs(totalImpact?.costPercentage || 0), 100)}%` }}
                    ></div>
                  </div>
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Schedule Impact</span>
                    <span className={`text-sm font-bold ${
                      selectedChangeOrder.scheduleImpact >= 0 
                        ? 'text-red-600 dark:text-red-400' 
                        : 'text-green-600 dark:text-green-400'
                  ...
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3">
                    <div
                      ...
                      style={{ width: `${Math.min(Math.abs(selectedChangeOrder.scheduleImpact) * 2, 100)}%` }}
                    ></div>
                  </div>
                </div>
              </div>
              ...
              <div className="border-t border-gray-200 dark:border-gray-700 pt-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className={`text-lg font-bold ${
                    ...
                    {formatCurrency(selectedChangeOrder.budgetImpact.variance)} ({selectedChangeOrder.budgetImpact.variancePercentage.toFixed(2)}%)
                  </span>
                </div>
              </div>
            </div>
                  ...
                  <div className="flex">
                    ...
                    <div className="ml-3">
                      ...
                      <p className="mt-1 text-sm text-red-700 dark:text-red-300">
                        This change order impacts the project's critical path and will affect the overall project timeline.
                      </p>
                    </div>
                  </div>
                </div>
                      ...
                      {selectedChangeOrder.scheduleImpactDetail.affectedTasks.map((task) => (
                        <tr key={task.taskId}>
                          ...
                          <td className={`px-6 py-4 whitespace-nowrap text-sm text-right font-medium ${
                            ...
                            {task.delayDays >= 0 ? '+' : ''}{task.delayDays}
                          </td>
                        </tr>
                      ))}
                    </tbody>
