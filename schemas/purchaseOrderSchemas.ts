...
export const purchaseOrderSchema = z.object({
  ...
  paymentTerms: z.string()
    .max(200, 'Payment terms is too long')
    .default('Net 30'),
 ...
 * Calculate PO total
 */
export function calculatePOTotal(items: POItemData[], shippingCost: number = 0): {
  subtotal: number;
  totalTax: number;
...
export function validatePONumber(poNumber: string): {
  ...
  if (!/^[A-Z0-9-]+$/.test(poNumber)) {
    return { valid: false, error: 'PO number can only contain uppercase letters, numbers, and hyphens' };
  }
